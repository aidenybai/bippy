...
describe("matchStateSpace", () => {
  ...
  it("locates the state of two repeats over one list, which decide a single count", () => {
    const shared = enumerateStateSpace([
      [fiber("ul", [repeat("items", [fiber("li")]), fiber("hr"), repeat("items", [fiber("b")])])],
    ]);
    expect(shared.stateCount).toBe(3);

    const agreeing = matchStateSpace(shared, [
      host("ul", [host("li"), host("li"), host("hr"), host("b"), host("b")]),
    ]);
    expect(agreeing.matched?.index).toBe(2);
    expect(describeConditions(agreeing.matched?.conditions ?? [])).toBe("items×2 items×2");

    const disagreeing = matchStateSpace(shared, [
      host("ul", [host("li"), host("hr"), host("b"), host("b")]),
    ]);
    expect(disagreeing.matched).toBeNull();
  });
