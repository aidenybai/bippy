...
// the first call runs synchronously. `isEqual` and the type predicates are
// decided over static values directly: lodash's own source runs natively only
// on arguments the analysis can lower, and a React element or a program function
// has a decided type without being lowerable. Every other helper runs from
// lodash's own source as a pure package or is analyzed when the package is
// allow-listed.
