...
export class Interpreter {
  ...
  private createObservedLanguages(): StaticValue | null {
    ...
    if (languages !== undefined)
      return listValue(languages.map((language) => primitiveValue(language)));
  ...
  private evaluateArrayExpression(node: ArrayExpression, context: EvaluationContext): StaticValue {
    ...
    for (const element of node.elements) {
      ...
      if (element.type === "SpreadElement") {
        ...
        items.push(...spreadListItems(getIterableItems(this, spread, context, location), location));
        continue;
      }
  ...
  private assignDynamicMember(
    ...
    if (keyNames.every((name) => name !== null)) {
      ...
      for (const name of new Set(keyNames))
        this.assignProperty(object, name, value, uncertainContext);
      return;
    }
    for (const alternative of object.kind === "branch" ? object.alternatives : [object]) {
      ...
      switch (child.type) {
        ...
        case "JSXExpressionContainer":
          if (child.expression.type !== "JSXEmptyExpression") {
            const value = this.evaluateExpression(child.expression, context);
            values.push(value.kind === "repeat" ? listValue([value]) : value);
          }
          break;
        case "JSXSpreadChild": {
