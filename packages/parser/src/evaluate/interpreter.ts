...
import { getModuleEnvironment, hasExportedName } from "../graph/module-record.js";
...
export class Interpreter {
  ...
  private readonly hostScalarGlobals = new Map<string, StaticValue>();
  ...
  private getModuleRegistry(
    ...
    return getModuleEnvironment(module, environment) === "server"
      ? this.serverRegistry
      : this.clientRegistry;
  }
  ...
  evaluateModuleExport(
    ...
    return this.resolvedSymbolToValue(
      this.graph.resolveExport(module, exportedName, environment),
      exportedName,
      environment,
    );
  }
  ...
  private getNamespaceMember(
    ...
    const { names, complete } = this.graph.collectExportNames(module, environment);
  ...
  private initializeDependencies(
    ...
    for (const specifier of module.dependencies) {
      const target = this.graph.resolveImportedModule(specifier, module, environment);
  ...
  private evaluateTopLevelBindingValue(
    ...
    switch (binding.kind) {
      ...
      case "import":
        return this.resolvedSymbolToValue(
          this.graph.resolveImport(binding.binding, module, context.environment),
          binding.name,
          context.environment,
        );
      case "destructured": {
  ...
  private getGlobal(name: string, renderEnvironment: RenderEnvironment | null): StaticValue | null {
    ...
    const pageLocationMember = realm.hasGlobal("location")
      ...
      : null;
    if (pageLocationMember) return pageLocationMember;
    const scalarKey = `${renderEnvironment ?? "client"}:${hostName}`;
    const memoized = this.hostScalarGlobals.get(scalarKey);
    if (memoized) return memoized;
    const builtin = getBuiltinGlobal(
      hostName,
      realm,
    ...
    if (builtin?.kind === "unknown-primitive") this.hostScalarGlobals.set(scalarKey, builtin);
    return builtin;
  }
  ...
  importModule(
    ...
    const target = this.graph.resolveImportedModule(specifier, context.module, context.environment);
    if (isModuleRecord(target)) {
      return isRequire
  ...
  private evaluateCallExpression(
    ...
    const getCallee = (target: StaticValue): StaticValue => {
      ...
      const name = getPropertyName(key);
      if (name === null) return unknownValue("computed method call", location);
