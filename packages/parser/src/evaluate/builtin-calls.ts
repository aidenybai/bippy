...
const callGlobal = (
  ...
  switch (name) {
    ...
    case "Array.from": {
      const fromSource = (alternative: StaticValue): StaticValue => {
        const source = iterableOrArrayLike(interpreter, alternative, context, location);
      ...
      return mapValue(first ?? UNDEFINED_VALUE, fromSource);
    }
    case "Int8Array.from":
