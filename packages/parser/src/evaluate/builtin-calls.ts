...
const callGlobal = (
  ...
  switch (name) {
    ...
    case "Array.from": {
      const dynamicIterable = unknownValue("Array.from of dynamic iterable", location);
      return mapValue(first ?? UNDEFINED_VALUE, (candidate) => {
        const source = iterableOrArrayLike(interpreter, candidate, context, location);
        if (source === null) return dynamicIterable;
        return mapValue(source, (items) => {
          if (items.kind !== "list" && items.kind !== "repeat") return dynamicIterable;
