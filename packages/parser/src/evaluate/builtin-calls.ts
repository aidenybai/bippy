...
import { getBuiltinPrototype } from "./instance-of.js";
...
export const getGlobalTypeof = (
  ...
  if (name.includes(".")) {
    if (environment === "server") return null;
    if (isWebCryptoName(name)) return "object";
    const [root, member, ...rest] = name.split(".");
    const isWindowGlobal =
      root !== undefined && member !== undefined && rest.length === 0 && isWindowAlias(root);
    return isWindowGlobal && GLOBAL_NAMES.has(member)
      ? getGlobalTypeof(member, environment)
      : getBrowserMemberTypeof(name);
  }
...
const callGlobal = (
  ...
  switch (name) {
    case "Date": {
      ...
      const date = constructNativeDate(args);
      if (date) return date;
  ...
  if (name.startsWith("Math.")) {
    ...
    if (
      ...
      if (method === "random") return rangedNumberValue(name, { min: 0, max: 1 });
      const compute = MATH_FUNCTIONS[method];
      if (compute) return primitiveValue(compute(...numbers));
    }
...
const MATH_FUNCTIONS: Record<string, (...numbers: number[]) => number> = {
  abs: Math.abs,
  acos: Math.acos,
  acosh: Math.acosh,
  asin: Math.asin,
...
export const evaluateBuiltinCall = (
  ...
  if (receiver.kind === "global") {
    ...
    if (isIndexedDbName(receiver.name)) {
      const requested = callIndexedDbMethod(
    ...
    if (isWebCryptoName(receiver.name)) {
      const result = callWebCryptoMethod(
        receiver.name,
        name,
        args,
