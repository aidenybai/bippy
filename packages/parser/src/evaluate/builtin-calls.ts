...
const cssSupportsValue = (interpreter: Interpreter, args: StaticValue[]): StaticValue => {
  ...
  return isSupported === null
    ? unknownPrimitiveValue(
        "boolean",
        `CSS.supports(${conditions.join(", ") || "dynamic condition"})`,
      )
...
const callGlobal = (
  ...
  switch (name) {
    ...
    case "Array.from": {
      ...
      const source = getArrayFromSource(iterated);
...
const filterItem = (
  ...
  const outcomes = alternatives.map((alternative) =>
    callCallback(interpreter, predicate, [alternative, index, list], context),
  );
...
const callStringMethod = (
  ...
  if (name === "split" || name === "replace" || name === "replaceAll") {
    ...
    if (second?.kind !== "primitive") {
      return second
        ? replaceWithDynamicText(receiver, pattern, second, name === "replaceAll")
        : null;
    }
