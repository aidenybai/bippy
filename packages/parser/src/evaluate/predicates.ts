...
/** The predicate of a materialized branch position: the same decision each time `owner` renders its `ordinal`-th branch. */
export const getPathPredicate = (
  owner: object,
  ordinal: number,
  reason: string,
  location: SourceLocation | null,
  ...
  choicePredicate({
    id: `${toInputId(getSubjectId(owner))}/${ordinal}`,
    label: reason,
    source: "path",
    location: location && formatSourceLocation(location),
  });
