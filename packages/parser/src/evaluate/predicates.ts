...
/** `test` with its `!` and `Boolean()` layers peeled: the value they negate and whether an odd number of `!` were applied. */
...
/** The `x === null`/`x != undefined` test `value` records, or null. */
export const getNullishTest = (value: StaticValue): EqualityDerivation | null => {
  const derivation = derivations.get(value);
  return derivation?.kind === "equality" &&
    (derivation.literal === null || derivation.literal === undefined)
    ? derivation
    : null;
};
...
/** The predicate taking the other side of a two-way `predicate`; null for an N-way choice. */
export const getNegatedPredicate = (predicate: string): string | null => {
  const parsed = parseSymbolicPredicate(predicate);
  return parsed.formula === null
    ? null
    : serializeSymbolicPredicate({ ...parsed, formula: negateGuard(parsed.formula) });
};
