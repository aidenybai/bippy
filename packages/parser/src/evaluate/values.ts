...
/**
 * Joining paths that each rewrite a reference (`registry = registry.filter(...)`)
 * doubles its alternatives per join; past this width the value is unknown, as
 * no alternative could be told apart usefully anymore.
 */
...
export const branchValue = (
  ...
  if (flattened.length > MAX_BRANCH_ALTERNATIVES) {
    return unknownValue(`${reason}: ${flattened.length} alternatives`, location);
  }
  const isPositional =
