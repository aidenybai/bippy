...
/** The entries each `Class.prototype` held once its own members were bound; any other entry was assigned onto it. */
const prototypeMemberEntries = new WeakMap<StaticObjectValue, Set<StaticObjectEntry>>();
...
const getPrototypeAssignments = (classValue: StaticClassValue): StaticObjectEntry[] => {
  ...
  const memberEntries = prototype && prototypeMemberEntries.get(prototype);
  return prototype && memberEntries
    ? prototype.entries.filter((entry) => !memberEntries.has(entry))
    : [];
};
...
export const getClassPrototypeObject = (
  ...
  prototypeMemberEntries.set(prototype, new Set(prototype.entries));
  return prototype;
};
