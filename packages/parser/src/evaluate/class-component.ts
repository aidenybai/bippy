...
/** The static getter React would invoke reading `Component.key`: the component's own, or one inherited through the constructor chain. */
export const getComponentStaticGetter = (
  component: ComponentDefinition,
  key: string,
): StaticFunctionValue | null => {
  const own = component.staticGetters.get(key);
  if (own) return own;
  const superValue = component.classBody?.superValue;
  return superValue?.kind === "class" ? getStaticGetter(superValue, key) : null;
};
