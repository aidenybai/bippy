import type { ClockTask, StaticValue } from "../types.js";
...
export class TimerQueue {
  ...
  constructor(private readonly settleMs = DEFAULT_SETTLE_MS) {}
...
const UNBOUNDED_ELAPSED: StaticValue = {
  ...
  clock: { ordering: "unbounded", sequence: 0, task: { scheduledBy: null, delayMs: 0 } },
};
...
/** The delays a later task's scheduling chain adds up to since an earlier task, or 0 when the earlier task is not on that chain. */
const getMinimumElapsed = (later: ClockTask, earlier: ClockTask): number => {
  let elapsed = 0;
  for (let task: ClockTask | null = later; task; task = task.scheduledBy) {
    if (task === earlier) return elapsed;
...
const subtractReadings = (left: StaticValue, right: StaticValue): StaticValue | null => {
  ...
  return leftClock.sequence >= rightClock.sequence
    ? rangedNumberValue(ELAPSED_REASON, {
        min: getMinimumElapsed(leftClock.task, rightClock.task),
        max: Number.POSITIVE_INFINITY,
      })
    : rangedNumberValue(ELAPSED_REASON, {
        min: Number.NEGATIVE_INFINITY,
        max: -getMinimumElapsed(rightClock.task, leftClock.task),
      });
};
