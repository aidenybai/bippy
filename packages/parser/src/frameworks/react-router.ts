...
const composeChain = (
  ...
  for (let index = chain.length - 1; index >= 0; index -= 1) {
    ...
    outlet = renderedRoute(outlet, params, renderRoute(route, outlet), route.id, hasRenderedRoute);
  }
  return outlet;
};
...
const renderMatchedRoutes = (
  ...
  const matched = composeChain(
    chain,
    renderDataRoute,
    hasRenderedRouteComponent(tools.project.readInstalledVersion("react-router")),
  );
...
const withinRouter = (children: StaticValue, location: StaticValue): StaticElementValue =>
  element(
    ...
    objectFromRecord({
      value: objectFromRecord({ location, navigationType: primitiveValue("POP") }),
      children,
    }),
  );
