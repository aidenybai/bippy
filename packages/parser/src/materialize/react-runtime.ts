...
const readAppReactDomVersion = (
  resolver: ModuleResolver | null,
  rootDirectory: string | null,
): string | null =>
  resolver !== null && rootDirectory !== null
    ? (readInstalledPackage(resolver, rootDirectory, "react-dom")?.version ?? null)
    : null;
...
const load = async (
  ...
  return {
    ...
    reconcilerVersion: readAppReactDomVersion(resolver, rootDirectory) ?? react.version,
  };
};
