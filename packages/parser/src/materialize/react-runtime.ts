...
const importReactModules = async (
  appResolver: ModuleResolver | null,
  rootDirectory: string | null,
...
const loadReactModules = async (
  ...
  const appModules = await importReactModules(
    hasOwnReact(resolver, rootDirectory) ? resolver : null,
    rootDirectory,
  );
  if (appModules.appResolver === null || hasHooks(appModules.react)) return appModules;
  return importReactModules(null, rootDirectory);
};
...
const load = async (
  ...
  const { react, dom, appResolver } = await loadReactModules(resolver, rootDirectory);
  return {
    react,
    dom,
