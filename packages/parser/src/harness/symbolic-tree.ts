...
/**
 * Per-alternative guards of a marker predicate: `[g, not g]` for a formula,
 * `eq(choice, i)` for a choice. The last alternative is what remains once the
 * others are excluded, so a choice never ranges outside its alternatives.
 */
export const predicateGuards = (
  ...
  if (predicate.choice) {
    ...
    const chosen = Array.from({ length: alternativeCount - 1 }, (_, index) =>
      choiceGuard(choice, index),
    );
    return [...chosen, negateGuard(orGuard(chosen))];
  }
