...
Live-verified so far: 104 entries are `exact` — the runtime capture is one of the enumerated
...
`react-material-admin`, `mantine-admin`, `mantine-react-table`), the Creative Tim/Tailwind
dashboard templates, `teable`, `magic-portfolio` and `shadcn-crm-dashboard`. 63 are `partial`:
...
1 `unsound` (`typebot`: replayed assignments of the `@tolgee/react` readiness branch reach a
`SignInForm` alternative the enumeration left unmaterialized), 6 `unresolved` and 12 render statically only (no dev server here). Provider packages
