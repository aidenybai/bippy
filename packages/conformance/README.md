...
Dispatch-accessor replays run forty-eight cases twice across the same four events. During callback lookup, a getter replaces its own property, adds a listener, optionally cancels/re-registers a pending sibling, and can emit a nested event. It returns the original callback, returns `undefined`, or throws. The outer delivery must use that returned value; nested/later delivery sees the replacement. Proxy traps verify accessor and callback receivers, single reads, exact arguments, root registration and both alternate IDs. Foreign/disposed registrations must not have their getters read. Getter, callback and reporter exceptions cannot suppress later observers or final unmount release. The original thirty-two callback/throw cases failed before the fix: Bippy reread the property and invoked the replacement, or let the getter escape its error boundary. The sixteen absent-return cases additionally prohibit invoking an installed replacement when the current lookup returns `undefined`. `core.ts` now performs optional method invocation inside `callListener`, preserving the receiver and evaluating the property once under the existing error guard. These directly exercise Bippy's event dispatch contract, not React scheduling or activation/metadata accessors.
...
pnpm test --project conformance dispatch-membership-replay dispatch-accessor-replay
...
- Add Bippy conformance and adversarial regression tests in `tests/`. Keep a replay when it reproduces a Bippy defect or covers a distinct API boundary; do not duplicate React's callback queues, effect publication or scheduling tests. Extend an existing case rather than copying it into another suite.
