...
Hidden-class callback replays run ten cases twice across two nested Activities: normal reveal, a throwing deferred callback, or deletion while hidden, with either root as the subject. Reveal cases also run with a reconnection-lifecycle update. Native queue audits require three exact callback references in append order, shared queue ownership across alternates, and unchanged retained batch arrays. Both Activity modes and their Offscreen children's hidden states are checked directly: revealing the inner boundary while the outer remains hidden cannot reconnect refs/lifecycles or deliver callbacks. On outer reveal, the deferred batch is cleared before delivery; a throwing callback suppresses the rest of that batch but not the separately queued ordinary callback. Pending fiber lanes prove the lifecycle update was actually scheduled even when error recovery abandons it; successful reveal commits it afterward. Repeat hiding/reveal cannot redeliver callbacks. Hidden deletion releases IDs without repeating disconnected cleanup, and late updates on the retired class cannot drive a fresh replacement. Exact traces retain each urgent unchanged-tree commit before the hidden background update rather than suppressing it. No Bippy production change was needed.
