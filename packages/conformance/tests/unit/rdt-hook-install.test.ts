...
import { ReactBuildType } from "../../../bippy/src/react-internals/index.js";
import {
  _onActiveListeners,
  getRDTHook,
  hasRDTHook,
...
import type { ReactDevToolsTarget } from "../../../bippy/src/rdt-hook.js";
import type {
  FiberRoot,
  ReactDevToolsGlobalHook,
  ReactRenderer,
} from "../../../bippy/src/react-internals/index.js";
...
it("checkDCE should schedule an error for badly built browser react", () => {
  const rdtHook = getRDTHook();
  vi.useFakeTimers();
...
it("checkDCE does not schedule a fatal error for unbundled Node entrypoints", () => {
  vi.useFakeTimers();
  try {
    getRDTHook().checkDCE(() => "^_^");
