...
## Namespace symbol properties and registry identity

The [namespace checks](../tests/namespace-exports.test.ts) now require the exact native `yes` for `Object.hasOwn(namespace, Symbol.toStringTag)`. For a modeled ESM namespace and a known symbol key, `hasOwnProperty` recognizes the specified own `%Symbol.toStringTag%` property; other symbols are absent. `propertyIsEnumerable` returns false for symbol keys. CommonJS-backed namespace representations and non-symbol keys do not use this rule. No namespace is materialized into an ordinary export object to answer these queries.

That rule requires symbol identity, not a matching description. The [registry operation](https://tc39.es/ecma262/#sec-symbol.for) creates `Symbol.for("Symbol.toStringTag")` independently of the well-known `Symbol.toStringTag`. Registry names that overlap well-known names, allocation IDs or the escape prefix now receive distinct internal keys. Descriptions and diagnostic formatting retain the original registry strings, including when symbol keys are reconstructed during object enumeration. The [registry checks](../tests/registered-symbols.test.ts) cover repeated registration, reserved names, allocation collisions and escaped-prefix collisions. Ordinary registry keys, including [React's registered element/type tags](https://github.com/facebook/react/blob/82c44beb444eda5230c063eaa163d01f38817211/packages/shared/ReactSymbols.js), retain their existing identities.

Two new native component cases check namespace symbol ownership/enumerability and distinct symbol-keyed entries with reconstructed descriptions. The original symbol fixture uses its unchanged saved native capture as an additional strict regression. Initial test fixtures used a single text child, which did not expose a HostText fiber to the text extractor; the retained initial failures were corrected with mixed host children before the semantic baseline was rerun. That fixture correction is not an analyzer repair.

The frozen `a7a026a9` plus nine-file candidate passes 1,967 analyzer tests and 4,044 root tests, retaining 38 existing expected failures and two existing skips. The 80 earlier exact saved checks remain unchanged, the former symbol limit becomes exact, and the two new native cases are exact: 83 strict/exact checks and 24 retained strict limits. Full application and eight prior native namespace models remain unchanged; 53 workflows remain partial and ten corpus controls are unchanged. The receipt is `/tmp/bippy-many-games-causal-investigation/namespace-symbols-reviewed-gates.json`. Initial overlapping corpus-lane launches and a workflow check before model readiness remain recorded; corrected runs complete the evidence. Initial model and successful lane-zero launcher exit statuses were not recorded, so their serialized artifacts are verified directly rather than presented as process-exit or cleanup evidence.

This does not implement the namespace's complete exotic-object behavior, symbol-value reads, descriptors, writes, live bindings, proxy descriptor traps, or general registry-key coercion. General string/symbol property-key storage collisions are separate from registry-symbol identity. Non-symbol namespace own-key queries retain their previous limits. Successful own-property checks do not establish that a module loaded or instantiated successfully.
