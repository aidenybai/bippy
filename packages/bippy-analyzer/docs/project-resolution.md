...
The existing native adapters remain differential-test tools and lower-level integrations, not alternative APIs an application must install. The [dual-context discovery audit](../corpus/module-resolution/project-discovery-audit.md) records both outcomes for all 2,299 requests across ten pinned repositories: 2,215 resolve in both contexts, 84 are Node-only, and 43 of the dual resolutions select different targets. These are discovery results, not execution acceptance. Its unchanged primary native comparison still matches 2,278/2,299; nine browser rejections and twelve Rollup externalization differences remain nonpasses. The earlier [configuration-execution audit](../corpus/module-resolution/project-execution-audit.md) and [static-only baseline](../corpus/module-resolution/project-audit.md) remain preserved.
