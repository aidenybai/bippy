...
const createCallbackCase = (
  ...
  const result = isReduce ? "total + (value ?? 0)"
    : method === "map" ? "value * 2"
      : method === "every" ? "value !== 99" : "value === 99";
  return {
...
describe.each(methods)("native $method callback semantics", ({ method, knownDivergences }) => {
  ...
  it.fails.each(knownDivergences)(
    "known divergence: %s mutation and callback traces",
    (mutation) => checkCallbacks(method, mutation),
  );
});

it.each(differentialSeeds)("matches native dense array searches at integral boundaries, seed %i", async (seed) => {
...
it.fails.each(["NaN", "1.8", "-1.8"])("known divergence: indexOf normalizes %s to an integer offset", (fromIndex) =>
  checkDifferentialCases([{ name: `indexOf(${fromIndex})`, body: `return [1, 1, 1].indexOf(1, ${fromIndex});` }]),
);

it.fails.each(["undefined", "null", "true", "false", "'1'"])("known precision gap: array searches coerce %s", (fromIndex) =>
  checkDifferentialCases([{ name: `search(${fromIndex})`, body: `return [1, 1, 1].indexOf(1, ${fromIndex}) + ':' + [1, 1, 1].includes(1, ${fromIndex});` }]),
);
...
it.each(differentialSeeds)("matches native mutation sequences and return values through aliases, seed %i", async (seed) => {
  const getRandom = createSeededRandom(seed);
  const cases: DifferentialCase[] = [];
  for (let index = 0; index < 60; index++) {
    ...
    for (let step = 0; step < 12; step++) {
      ...
      const operations = [
        `values.push(${value})`, `alias.unshift(${value})`, "values.pop()", "alias.shift()",
        "values.reverse()", "alias.sort((first, second) => first - second)",
        `values.splice(${start}, ${getRandom(4)}, ${value})`, `alias.slice(${start})`,
        `values.concat([${value}])`, `values.fill(${value})`, "alias.toReversed()", `values.at(${start})`,
      ];
