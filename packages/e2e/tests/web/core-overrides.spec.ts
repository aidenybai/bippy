...
test.describe("secure", () => {
  test("secured handlers still fire on a development renderer", async ({ page }) => {
    const result = await page.evaluate(() => {
      return new Promise<{ commitFired: boolean }>((resolve) => {
        const securedOptions = window.__BIPPY__.secure({
          onCommitFiberRoot: () => {
            resolve({ commitFired: true });
          },
        });
        window.__BIPPY__.instrument(securedOptions);
        document.querySelector<HTMLElement>('[data-testid="increment"]')!.click();
      });
    });
    expect(result.commitFired).toBe(true);
  });

  test("routes handler exceptions to onError instead of throwing", async ({ page }) => {
    const result = await page.evaluate(() => {
      return new Promise<{ sawError: boolean; message: string }>((resolve) => {
        const securedOptions = window.__BIPPY__.secure(
          {
            onCommitFiberRoot: () => {
              throw new Error("intentional-e2e-error");
            },
          },
          {
            onError: (error) => {
    ...
    expect(result.message).toBe("intentional-e2e-error");
  });
});
