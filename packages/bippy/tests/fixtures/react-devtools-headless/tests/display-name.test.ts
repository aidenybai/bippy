...
import { extractHOCNames } from "../src/display-name.js";

describe("upstream HOC display-name behavior", () => {
  it("should return an empty result for an empty display name", () => {
    expect(extractHOCNames("")).toEqual({ baseComponentName: "", hocNames: [] });
  });

  it("should not extract anything from an unwrapped component", () => {
    expect(extractHOCNames("Button")).toEqual({ baseComponentName: "Button", hocNames: [] });
  });
