...
3. Root validation passes **2,899 tests**, with two existing React-19 DevTools skips; this includes **853 parser tests / 49 files**. Root typecheck/build, realm checks, lint and formatting pass. The latest serial corpus run takes 27.451 seconds for Sentry and 164.794 seconds for PostHog. Current tooling uses Node 24.21.0; timings are not a controlled comparison with earlier environments. Performance variability remains unresolved. P1/P2 and the 500-repository gate remain incomplete.
