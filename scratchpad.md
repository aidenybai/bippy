...
3. Root validation passes **2,962 tests**, with two existing React-19 DevTools skips; this includes **916 parser tests / 54 files**. Root typecheck/build, realm checks, lint and formatting pass. The latest serial corpus run takes 26.694 seconds for Sentry and 161.985 seconds for PostHog. Current tooling uses Node 24.21.0; timings are not a controlled comparison with earlier environments. Performance variability remains unresolved. P1/P2 and the 500-repository gate remain incomplete.
