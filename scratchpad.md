...
3. Root validation passes **2,908 tests**, with two existing React-19 DevTools skips; this includes **862 parser tests / 50 files**. Root typecheck/build, realm checks, lint and formatting pass. The latest serial corpus run takes 42.396 seconds for Sentry and 170.638 seconds for PostHog. Current tooling uses Node 24.21.0; timings are not a controlled comparison with earlier environments. Performance variability remains unresolved. P1/P2 and the 500-repository gate remain incomplete.
...
### Money, todo and design-system batch in progress
...
- `reactjs-github-blog-challenge` at `2227c94fcfcf7b7fce5ca30283f06c9a7af00343` has no runnable package manifest and is not a corpus entry. The committed count remains 215; 285 configurations remain before the 500-repository gate.
