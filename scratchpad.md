...
3. Root validation passes **2,945 tests**, with two existing React-19 DevTools skips; this includes **899 parser tests / 54 files**. Root typecheck/build, realm checks, lint and formatting pass. The latest serial corpus run takes 26.883 seconds for Sentry and 162.361 seconds for PostHog. Current tooling uses Node 24.21.0; timings are not a controlled comparison with earlier environments. Performance variability remains unresolved. P1/P2 and the 500-repository gate remain incomplete.
