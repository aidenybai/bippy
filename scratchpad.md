...
### UI24 post-read reconciliation: thirteen authored-text reviews and recaptures
...
Reconciliation bundles0–2 were actually read to completion in bounded, untruncated ranges. Five first-group supplemental chunks and three remaining-group chunks then covered omitted tracked text and resolved duplicate references against bodies displayed in this continuation—not the earlier unsupported UI28 receipt. All13 imported UI24 repositories now have retrospective authored-text review covering177 tracked text files:90 directly read distinct bodies and87 verified byte-identical references. Fourteen lockfiles retain machine dependency audits rather than line-reading claims;38 binary assets are retained without visual-review claims. The two installed Values2.0.0/2.1.1 ESM implementations were also read. This does not certify all transitive dependency source, pixels or original pre-setup review.
...
### UI25 post-read reconciliation: thirteen more reviews, no new imports

UI25's13 imported repositories now have retrospective authored-text review covering161 tracked text files:93 directly displayed distinct bodies (84 from19 bounded supplement chunks and nine from the previously completed reconciliation bundle2) plus68 byte-identical references to bodies actually read. Thirteen lockfiles remain machine-audited rather than line-read, and22 binary assets remain outside visual review. Original source, CSS, configuration, IDE XML, licenses and deployment files were preserved. No full transitive-dependency or original pre-setup reading claim is added.
