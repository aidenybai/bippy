...
3. Root validation passes **2,888 tests**, with two existing React-19 DevTools skips; this includes **842 parser tests / 48 files**. Root typecheck/build, realm checks, lint and formatting pass. The latest serial corpus run takes 27.031 seconds for Sentry and 173.610 seconds for PostHog. Current tooling uses Node 24.21.0; timings are not a controlled comparison with earlier environments. Performance variability remains unresolved. P1/P2 and the 500-repository gate remain incomplete.
